...
angular.module('MeanApp')
  ...
  .directive('crudTable1',['Modal', '$rootScope','User', 'Auth','$injector','$loading','socket','toastr','$cookieStore', function (Modal,$rootScope,User,Auth,$injector,$loading,socket,toastr,$cookieStore) {
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        scope.data = api.my.query({},function(res){
          ...
          for (var i=0;i<scope.data.length;i++)
          {
            ...
            if((scope.data[i].in=='association')==false)
            {

              count++
              scope.notifiction=count


            }
            else if((scope.data[i].in=='doctor').valueOf(false))
            {
              count=count+1

            }
          }
          console.log(scope.notifiction)
        });
        //scope.$on('notifiction', function () {socket.syncUpdates(attrs.api.toLowerCase(), scope.data); });
        // scope.$watch('notifiction', function() {
        //    alert('another patient is already taken by a doctor');
        //  });
        ...
        // console.log(scope.getCurrentUser.email)
        ...
        scope.$on('$destroy', function () {socket.unsyncUpdates(attrs.api.toLowerCase());
        });
      }
    };}])
  .directive('crudTable2',['Modal','$injector','$loading','socket','toastr', function (Modal,$injector,$loading,socket,toastr) {
    return {
      templateUrl: 'app/directive/tableCreate.html',
      restrict: 'EA',
      scope: {obj:'='},
      link: function (scope, element, attrs) {
        // var cols = ['name','info','parent','image'];
  ...
  .directive('crudTable3',['Modal', '$rootScope','User', 'Auth','$injector','$http','$loading','socket','toastr','$cookieStore', function (Modal,$rootScope,User,Auth,$injector,$http,$loading,socket,toastr,$cookieStore) {
    return {
      templateUrl: 'app/directive/trackpatient.html',
      restrict: 'EA',
      scope: {obj:'='},
      link: function (scope, element, attrs) {
        // var cols = ['name','info','parent','image'];
        ...
        /*$http.get('api/patients/showDoctor/' + scope.api.doctor).then(function(response){
